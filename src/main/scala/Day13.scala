package me.dalsat.adventofcode.day14
...
object Day13 extends InputLoader(13) {

  //  val bs = BusScheduler(sample)
  val bs = BusScheduler(input)

  println(bs.arrivalTime)
  println(bs.busList)
  ...
  println(s"result=${bs.findPart2Timestamp}")
...
class BusScheduler(dataset: Dataset) {
  ...
  def findPart2Timestamp = {
    ...
    @tailrec
    def loop(timestamp: BigInt, currentStep: BigInt, unprocessedBusList: Seq[(Int, Int)]): BigInt = unprocessedBusList match {
      case (bus, index) :: tail if (timestamp + index) % bus == 0 =>
        println(s"timestamp ${timestamp} matches bus ${bus} with index ${index} -> new step: ${lcm(currentStep, bus)}")
  ...
  def gcd(first: BigInt, second: BigInt) = LazyList.iterate((first, second)) { case (x, y) => (y, x % y) }
